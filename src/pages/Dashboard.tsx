...
const Dashboard = () => {
  ...
  const loadStats = async () => {
    try {
      ...
      const [accountsRes, phrasesRes, postsRes, postsTodayRes] = await Promise.all([supabase.from("threads_accounts").select("id", {
        count: "exact"
      }), supabase.from("phrases").select("id", {
        count: "exact"
      }), supabase.from("periodic_posts").select("id", {
        count: "exact"
      }), supabase.from("post_history").select("id", {
        count: "exact"
  ...
  const loadPostHistory = async () => {
    try {
      const {
        data,
        error
      } = await supabase.from("post_history").select(`
          id,
          content,
          posted_at,
  ...
  return <Layout>
      <div className="space-y-8">
        ...
        <div className="grid gap-4 md:grid-cols-4">
          ...
          return <Card key={stat.title} className="relative overflow-hidden border-border/50 bg-card/50 backdrop-blur-sm hover:shadow-lg hover:shadow-primary/5 transition-all duration-300 hover:scale-105 group animate-scale-in" style={{
          ...
          <CardContent className="space-y-4 relative">
            <div className="space-y-3">
              ...
              <ul className="space-y-2">
                {[{
                text: "Conecte suas contas do Threads na aba 'Contas'",
                icon: Users
              }, {
                text: "Adicione frases que deseja postar na aba 'Frases'",
                icon: MessageSquare
              }, {
                text: "Configure posts periódicos na aba 'Posts Periódicos'",
                icon: Calendar
              }].map((step, index) => {
                const Icon = step.icon;
                return <li key={index} className="flex items-start gap-3 text-sm text-muted-foreground hover:text-foreground transition-colors duration-200 group" style={{
        ...
        <Card className="border-border/50 bg-card/50 backdrop-blur-sm animate-fade-in-delayed overflow-hidden">
          ...
          <CardContent className="relative">
            {postHistory.length === 0 ? <div className="text-center py-8">
                <MessageSquare className="h-12 w-12 text-muted-foreground/50 mx-auto mb-3" />
                <p className="text-sm text-muted-foreground">Nenhum post realizado ainda</p>
              </div> : <div className="space-y-3">
                {postHistory.map((post, index) => <div key={post.id} className="border-l-2 border-primary/50 pl-4 py-3 hover:border-primary hover:bg-primary/5 transition-all duration-300 rounded-r-lg group animate-fade-in" style={{
              animationDelay: `${index * 0.05}s`
                    ...
                    <div className="flex items-center gap-4 mt-2 text-sm text-muted-foreground">
                      ...
                      <span className="flex items-center gap-1.5 hover:text-primary transition-colors duration-200">
                        ...
                        {format(new Date(post.posted_at), "dd/MM/yyyy 'às' HH:mm", {
                    locale: ptBR
                  })}
                      </span>
                    </div>
